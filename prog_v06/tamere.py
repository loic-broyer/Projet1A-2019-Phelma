...
texte = open("Trackbar.txt","r")
pos = (texte.read()).split(",")

def nothing(x):
    pass
cv2.namedWindow('window')
cv2.createTrackbar('iterationDilatation','window',int(pos[0]),8,nothing)
cv2.createTrackbar('iterationOuverture','window',int(pos[1]),8,nothing)
cv2.createTrackbar('aireMin','window',int(pos[2]),10000,nothing)
cv2.createTrackbar('aireMax','window',int(pos[3]),10000,nothing)
cv2.createTrackbar('periMin','window',int(pos[4]),10000,nothing)
cv2.createTrackbar('periMax','window',int(pos[5]),10000,nothing)
cv2.createTrackbar('Hmin','window',int(pos[6]),179,nothing)
cv2.createTrackbar('Hmax','window',int(pos[7]),179,nothing)
cv2.createTrackbar('Smin','window',int(pos[8]),255,nothing)
cv2.createTrackbar('Smax','window',int(pos[9]),255,nothing)
cv2.createTrackbar('Vmin','window',int(pos[10]),255,nothing)
cv2.createTrackbar('Vmax','window',int(pos[11]),255,nothing)
#cv2.namedWindow('window')
#cv2.createTrackbar('iterationDilatation','window',int(0),8,nothing)
#cv2.createTrackbar('iterationOuverture','window',int(0),8,nothing)
...
while True:
    ...
    if key == 27:
        break
texte.write (str(nbIterationDilatation)+","+str(nbIterationOuverture)+","+str(aireMin)+","+str(aireMax)+","+str(periMin)+","+str(periMax)+","+str(Hmin)+","+str(Hmax)+","+str(Smin)+","+str(Smax)+","+str(Vmin)+","+str(Vmax)+",")
texte.close()
